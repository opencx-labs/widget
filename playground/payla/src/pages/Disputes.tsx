...
export function Disputes() {
  ...
  const columns: Column<Dispute>[] = [
    ...
    },
    {
      ...
      header: 'Reason',
      render: (d) => (
        <span className="text-ink-2">{statusLabel(d.reason)}</span>
      ),
    },
    {
      key: 'status',
      header: 'Status',
      render: (d) => (
        <StatusPill tone={disputeTone(d.status)} label={d.status} />
      ),
    },
    {
      key: 'due',
      header: 'Respond by',
      render: (d) => <span className="text-ink-2">{formatDate(d.dueAt)}</span>,
    },
    {
      key: 'amount',
      header: 'Amount',
      align: 'right',
      render: (d) => (
        <span className="font-medium text-ink">{formatMoney(d.amount)}</span>
      ),
    },
  ];
  ...
  return (
    <Card>
      <DataTable
        ...
        empty={
          <EmptyState icon={<ShieldCheck size={22} />} title="No disputes">
            You have no chargebacks or disputes. Nice.
          </EmptyState>
        }
      />
    </Card>
