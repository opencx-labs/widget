...
export function PaymentLinks() {
  ...
  const columns: Column<PaymentLink>[] = [
    ...
    },
    {
      ...
      header: 'Status',
      render: (l) => (
        <StatusPill tone={paymentLinkTone(l.status)} label={l.status} />
      ),
    },
    {
      key: 'created',
      header: 'Created',
      render: (l) => (
        <span className="text-ink-2">{formatDate(l.createdAt)}</span>
      ),
    },
    {
      key: 'url',
      ...
      render: (l) => (
        <div className="flex items-center gap-1">
          <span className="max-w-[180px] truncate font-mono text-[12px] text-ink-3">
            {l.url.replace(/^https?:\/\//, '')}
          </span>
    ...
    },
    {
      ...
      align: 'right',
      render: (l) => (
        <span className="font-medium text-ink">{formatMoney(l.amount)}</span>
      ),
    },
  ];
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-[13px] text-ink-3">
          Charge customers without an integration — share a link, get paid.
        </p>
      ...
      <Card>
        <DataTable
          ...
          loading={isLoading}
          empty={
            <EmptyState icon={<Link2 size={22} />} title="No payment links yet">
              Create your first link to start charging customers.
            </EmptyState>
          }
        />
      </Card>
