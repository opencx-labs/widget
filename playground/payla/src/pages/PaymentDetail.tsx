...
export function PaymentDetail() {
  ...
  const remaining = payment
    ? Number(payment.amount.value) - Number(payment.amountRefunded.value)
    : 0;
  ...
  return (
    <div className="space-y-5">
      <Link
        to="/payments"
        className="focusable inline-flex items-center gap-1.5 rounded-md text-[13px] text-ink-2 hover:text-ink"
      >
                ...
                <dl className="divide-y divide-border px-5 py-1">
                  ...
                  <KeyValue label="Refunded">
                    {Number(payment.amountRefunded.value) > 0
                      ? `−${formatMoney(payment.amountRefunded)}`
                      : '—'}
                  </KeyValue>
                  ...
                  <KeyValue label="Settlement">
                    {payment.settlementId ? (
                      <Link
                        to={`/settlements/${payment.settlementId}`}
                  ...
                  {payment.customerId ? (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="font-mono text-[12px] text-link">
                          {payment.customerId}
                        </div>
                      </div>
                    </Link>
