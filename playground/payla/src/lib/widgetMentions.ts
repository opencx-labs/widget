...
/** The widget's `context.entity` shape: what the current page "is". */
type Entity = {
  type: string;
  id: string;
  title: string;
...
/** An entity plus what the @-mention menu shows for it. */
type Mention = Entity & {
  description: string;
  iconName: 'CreditCard' | 'User';
};
...
export function currentEntity(pathname: string): Entity | undefined {
  const [, section, id] = pathname.split('/');
  if (!id) return undefined;
  if (section === 'payments')
    return { type: 'payment', id, title: `Payment ${id}` };
  if (section === 'customers')
    return { type: 'customer', id, title: `Customer ${id}` };
  if (section === 'settlements')
    return { type: 'settlement', id, title: `Settlement ${id}` };
  return undefined;
}
