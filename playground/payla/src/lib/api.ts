...
const paginated = <T extends z.ZodTypeAny>(item: T) =>
  z.object({
    data: z.array(item),
    meta: z.object({ count: z.number(), total: z.number() }),
  });
...
export const api = {
  ...
  refundPayment: (id: string, body: { amount?: number; reason?: string }) =>
    request(`/payments/${id}/refunds`, RefundResult, {
      method: 'POST',
      body: JSON.stringify(body),
    }),

  refunds: (limit?: number) =>
    request(`/refunds${qs({ limit })}`, paginated(Refund)),

  customers: (limit?: number) =>
    request(`/customers${qs({ limit })}`, paginated(Customer)),
  customer: (id: string) => request(`/customers/${id}`, CustomerWithPayments),

  paymentLinks: (limit?: number) =>
    request(`/payment-links${qs({ limit })}`, paginated(PaymentLink)),
  createPaymentLink: (body: { amount: number; description: string }) =>
    request('/payment-links', PaymentLink, {
      method: 'POST',
      body: JSON.stringify(body),
    }),

  settlements: (limit?: number) =>
    request(`/settlements${qs({ limit })}`, paginated(Settlement)),
  settlement: (id: string) => request(`/settlements/${id}`, Settlement),

  disputes: (limit?: number) =>
    request(`/disputes${qs({ limit })}`, paginated(Dispute)),
};
