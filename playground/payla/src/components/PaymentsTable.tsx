...
export function PaymentsTable({
  ...
  const columns: Column<Payment>[] = [
      ...
      ? [
          {
            ...
            header: 'Customer',
            render: (p: Payment) => (
              <span className="text-ink-2">{p.customerName ?? '—'}</span>
            ),
      ...
      : []),
    {
      ...
      header: 'Method',
      render: (p) => (
        <span className="text-ink-2">{methodLabel(p.method)}</span>
      ),
    },
    {
      key: 'date',
      ...
      render: (p) => (
        <span className="whitespace-nowrap text-ink-2">
          {formatRelative(p.paidAt ?? p.createdAt)}
        </span>
      ),
    },
    {
      ...
      header: 'Status',
      render: (p) => (
        <StatusPill tone={paymentTone(p.status)} label={p.status} />
      ),
    },
    {
      key: 'amount',
        ...
        <div className="whitespace-nowrap">
          ...
          {Number(p.amountRefunded.value) > 0 && (
            <div className="text-[12px] text-ink-3">
              −{formatMoney(p.amountRefunded)} refunded
            </div>
          )}
        </div>
      ),
