...
export function RefundDialog({
  ...
  const submit = async () => {
    ...
    if (value > remaining + 1e-9)
      return setError(
        `Amount can't exceed the refundable ${remaining.toFixed(2)}.`,
      );
    try {
  ...
  return (
    <Dialog open={open} onClose={onClose} title="Refund payment">
      <div className="space-y-4">
        ...
        <Field
          label="Amount"
          hint="Refunds go back to the customer's original payment method."
        >
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          >
            {refund.isPending
              ? 'Refunding…'
              : `Refund ${formatMoney({ value: (Number(amount) || 0).toFixed(2), currency: payment.amount.currency })}`}
          </Button>
        </div>
      </div>
    </Dialog>
