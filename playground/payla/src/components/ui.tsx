...
const BUTTON_VARIANTS: Record<ButtonVariant, string> = {
  ...
  secondary:
    'bg-surface text-ink border border-border-strong hover:bg-surface-3',
  ghost:
    'bg-transparent text-ink-2 hover:bg-surface-3 border border-transparent',
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-2 py-16 text-center">
      ...
      {children && (
        <p className="max-w-sm text-[13px] text-ink-3">{children}</p>
      )}
    </div>
  );
}
...
export function Skeleton({ className }: { className?: string }) {
  return (
    <div className={cn('animate-pulse rounded-md bg-surface-4', className)} />
  );
}
...
export function KeyValue({
  ...
  return (
    <div className="flex items-baseline justify-between gap-4 py-2.5">
      ...
      <dd className="text-[13px] font-medium text-ink text-right">
        {children}
      </dd>
    </div>
  );
