...
export function AreaChart({
  ...
  const x = (i: number) =>
    padX + (data.length <= 1 ? 0 : (i / (data.length - 1)) * innerW);
  const y = (v: number) => padTop + innerH - (v / max) * innerH;

  const line = data
    .map(
      (d, i) =>
        `${i === 0 ? 'M' : 'L'}${x(i).toFixed(1)},${y(d.amount).toFixed(1)}`,
    )
    .join(' ');
      ...
      {hover !== null && data[hover] && (
        <div
          ...
          style={{
            ...
            top: Math.max(y(data[hover].amount) - 52, 0),
          }}
        >
          <div className="text-sm font-semibold text-ink">
            {fmt.format(data[hover].amount)}
          </div>
