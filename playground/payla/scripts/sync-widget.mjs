...
function resolveSourceDirectory(source) {
  const candidate = resolve(source);
  if (existsSync(candidate) && statSync(candidate).isDirectory()) {
    return candidate;
  }
  // Backward compatible with the old OPENCX_WIDGET_DIST contract, which
  // pointed directly at dist-embed/script.js rather than its directory.
  return basename(candidate) === 'script.js' ? dirname(candidate) : candidate;
}

export function syncWidgetBuild({
  source = process.env.OPENCX_WIDGET_DIST ?? defaultSource,
  destination = defaultDestination,
  logger = console,
} = {}) {
  const sourceDirectory = resolveSourceDirectory(source);
  ...
  if (!existsSync(loaderEntry) || !existsSync(moduleEntry)) {
    ...
    logger.warn(
      `[sync-widget]   (or set OPENCX_WIDGET_DIST to dist-embed). Skipping — the app runs, the widget won't load until synced.`,
    );
    return false;
  }
