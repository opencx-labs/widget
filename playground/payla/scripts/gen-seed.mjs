...
const rows = [];
// The seed is the reset: it empties every table first, children before the
// tables they reference (refunds and disputes point at payments; payments at
// customers and settlements), so it runs clean on an empty database (as
// migration 0002) and again on a populated one (`pnpm db:seed:local`).
//
// Clearing first is safe without an explicit transaction: `wrangler d1
// execute --file` applies the whole file atomically, so a failed statement
// or a Ctrl-C rolls back to the data that was already there rather than
// leaving the demo half-seeded. Verified against the local D1 — a file that
// deletes every table and then fails on a duplicate key leaves the previous
// rows untouched. Do not add BEGIN/COMMIT here; it would only duplicate a
...
CUSTOMERS.forEach(([name, email, locale], i) => {
  ...
  stmt(
    'customers',
    ['id', 'name', 'email', 'locale', 'created_at'],
    [
      sql(id),
      sql(name),
      sql(email),
      sql(locale),
      sql(iso(now - int(40, 400) * DAY)),
    ],
  );
});
...
for (let i = 0; i < 6; i++) {
  ...
  const settledAt =
    status === 'paidout' ? iso(now - daysAgo * DAY + DAY) : null;
  ...
  stmt(
    ...
    [
      sql(id),
      sql(`1180.${String(1000 + i).slice(1)}.${2026}`),
      0,
      sql('USD'),
      sql(status),
      sql(iso(now - daysAgo * DAY)),
      sql(settledAt),
    ],
  );
}
...
for (let i = 0; i < N; i++) {
  ...
  if (isSettled && now - createdMs > DAY) {
    const s =
      pick(settlements.filter((x) => x.status !== 'open')) || settlements[2];
    settlementId = s.id;
  ...
  if (status === 'partially_refunded')
    refunded = Math.round(amount * pick([0.25, 0.5, 0.5, 0.33]));
  ...
  if (status === 'paid' || status === 'partially_refunded')
    paidPayments.push({ id, amount, createdMs, custId });
...
for (const s of settlements) {
  rows.push(
    `UPDATE settlements SET amount_cents = ${settlementCents[s.id]} WHERE id = ${sql(s.id)};`,
  );
}
...
LINKS.forEach(([desc, cents, status], i) => {
  ...
  stmt(
    ...
    [
      ...
      sql('USD'),
      sql(status),
      sql(`https://pay.payla.dev/${id.slice(3)}`),
      sql(iso(createdMs)),
      sql(paidAt),
    ],
  );
...
for (let i = 0; i < 4 && i < paidPayments.length; i++) {
  ...
  stmt(
    ...
    [
      ...
      p.amount,
      sql('USD'),
      sql(disputeReasons[i % disputeReasons.length]),
      sql(status),
      sql(iso(createdMs)),
      sql(
        status === 'open' || status === 'under_review'
          ? iso(createdMs + 14 * DAY)
          : null,
      ),
    ],
  );
...
const openTotal =
  settlementCents[settlements[0].id] + settlementCents[settlements[1].id];
const available = Math.round(openTotal * 0.35) + int(120_00, 480_00);
stmt(
  'balance',
  ['id', 'available_cents', 'pending_cents', 'currency'],
...
stmt(
  ...
  [
    1,
    sql("Mo's Coffee Roasters"),
    sql('org_payla_demo'),
    sql('owner@moscoffee.example'),
    sql('US'),
    sql('daily'),
    sql('MOS COFFEE'),
    0,
...
const header = `-- AUTO-GENERATED by scripts/gen-seed.mjs — do not edit by hand.\n-- Deterministic demo data for "Mo's Coffee Roasters". Safe to re-run: it\n-- clears every table first, so it is also the reset. wrangler applies the\n-- whole file atomically, so a failure rolls back rather than half-seeding.\n\n`;
