...
app.get('/api/payments/:id', async (c) => {
  ...
  if (!payment)
    return c.json(
      { error: { type: 'not_found', message: 'Payment not found.' } },
      404,
    );
...
app.post('/api/payments', async (c) => {
  const body = CreatePaymentInput.safeParse(
    await c.req.json().catch(() => ({})),
  );
  if (!body.success)
    return badRequest(c, 'Invalid payment payload.', body.error.flatten());
...
app.post('/api/payments/:id/refunds', async (c) => {
  ...
  const body = RefundPaymentInput.safeParse(
    await c.req.json().catch(() => ({})),
  );
  if (!body.success)
    return badRequest(c, 'Invalid refund payload.', body.error.flatten());
  ...
  const remainingCents =
    majorToCents(Number(payment.amount.value)) -
    majorToCents(Number(payment.amountRefunded.value));
  const requestedCents = body.data.amount
    ? majorToCents(body.data.amount)
    : remainingCents;
  if (requestedCents <= 0)
    return badRequest(c, 'Refund amount must be positive.');
  ...
  const refund = await c.var.db.createRefund(
    payment,
    requestedCents,
...
app.get('/api/customers', async (c) => {
  const { data, total } = await c.var.db.listCustomers(
...
app.get('/api/customers/:id', async (c) => {
  ...
  const payments = await c.var.db.listPayments({
...
app.get('/api/payment-links', async (c) => {
  const data = await c.var.db.listPaymentLinks(
    clampLimit(c.req.query('limit')),
  );
  return c.json({ data, meta: { count: data.length, total: data.length } });
});
...
app.post('/api/payment-links', async (c) => {
  const body = CreatePaymentLinkInput.safeParse(
    await c.req.json().catch(() => ({})),
  );
  if (!body.success)
    return badRequest(c, 'Invalid payment-link payload.', body.error.flatten());
  const link = await c.var.db.createPaymentLink(
...
app.get('/api/settlements/:id', async (c) => {
  ...
  if (!settlement)
    return c.json(
      { error: { type: 'not_found', message: 'Settlement not found.' } },
      404,
    );
...
app.get('/api/settings', async (c) => {
  ...
  if (!settings)
    return c.json(
      { error: { type: 'not_found', message: 'Settings not found.' } },
      404,
    );
