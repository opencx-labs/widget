...
export class PaylaDb {
  ...
  async listPayments(opts: {
    ...
    const res = await this.db
      ...
      .all<PaymentRow>();
    return {
      data: res.results.map(mapPayment),
      total: total?.n ?? res.results.length,
    };
  }

  async getPayment(id: string): Promise<Payment | null> {
    const row = await this.db
      .prepare(`${PAYMENT_SELECT} WHERE p.id = ?`)
      .bind(id)
      .first<PaymentRow>();
  ...
  async createRefund(
    ...
    const newRefunded =
      majorToCents(Number(payment.amountRefunded.value)) + amountCents;
    const paymentTotal = majorToCents(Number(payment.amount.value));
    const newStatus: PaymentStatus =
      newRefunded >= paymentTotal ? 'refunded' : 'partially_refunded';
    await this.db.batch([
      this.db
        .prepare(
      ...
      this.db
        .prepare(
          `UPDATE payments SET amount_refunded_cents = ?, status = ? WHERE id = ?`,
        )
  ...
  async listCustomers(limit: number) {
    ...
    const res = await this.db
      ...
      .all<CustomerRow>();
    return {
      data: res.results.map(mapCustomer),
      total: total?.n ?? res.results.length,
    };
  }
  ...
  async getBalance(currency = 'USD'): Promise<Balance> {
    const row = await this.db
      .prepare(
        `SELECT available_cents, pending_cents, currency FROM balance WHERE id = 1`,
      )
      .first<{
    ...
    return {
      available: centsToMoney(
        row?.available_cents ?? 0,
  ...
  async getMetrics(): Promise<Metrics> {
    ...
    const disputes = await this.db
      .prepare(
        `SELECT COUNT(*) AS n FROM disputes WHERE status IN ('open','under_review')`,
      )
      .first<{ n: number }>();
    ...
    return {
      ...
      successRate,
      avgOrderValue: centsToMoney(
        cur.count > 0 ? Math.round(cur.cents / cur.count) : 0,
      ),
