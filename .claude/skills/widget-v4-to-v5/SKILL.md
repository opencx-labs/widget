...
v5 is a pre-release on the npm `beta` dist-tag; `@latest` is still 4.0.x.
...
Pin the exact `5.0.0-beta.N` instead of the tag if you need a fixed build.
...
| `WidgetUserMessage.deliveredAt` was removed; `pending?`, `markedElements?` and `mentions?` were added.                                                                                                                          | `customComponents['message::after']`, `components` overrides, headless consumers reading user messages.                              | Use `timestamp` (same instant). Type-only change.                                                                                                                                                                                              |
...
initOpenScript({
  ...
  mentions: {
    items: [
      { type: 'plan', id: 'pro', title: 'Pro plan', iconName: 'CreditCard' },
    ],
...
(`chat/streaming_turn/*`, `chat/turn_failed/*`, `chat/clarification_questions/*`,
`chat/queued_sends/*`, `chat/input_box/page_mark_*`,
`chat/input_box/page_context_container`, `chat/input_box/page_context_pill`,
`chat/mention`, `chat/input_box/mention_picker*`, `companion/*`). New translation keys
can be overridden through `translationOverrides`.
