 ...
 *
 * The slim widget prompt documents no `repeat` directive, but models that know
 * the full json-render contract sometimes invent one anyway (bind a container's
 * children to a `/state` array) and apply it to a `List` — a leaf that renders
 * `props.items` and ignores `children` — producing
