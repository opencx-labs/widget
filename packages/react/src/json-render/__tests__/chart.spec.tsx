...
describe('Chart', () => {
  it('renders a bar per datum, with axes and grid', () => {
    ...
    expect(c.querySelectorAll('.recharts-bar-rectangle')).toHaveLength(
      data.length,
    );
    ...
    expect(tickLabels(c)).toEqual(
      expect.arrayContaining(['Mon', 'Tue', 'Wed']),
    );
  });
  ...
  it('renders a legend entry per datum plus the center label', () => {
    ...
    expect(c.querySelectorAll('.recharts-legend-item')).toHaveLength(
      data.length,
    );
