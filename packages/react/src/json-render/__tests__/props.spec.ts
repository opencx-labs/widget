...
describe('parseProps', () => {
  ...
  it('returns the fallback when a required prop is missing', () => {
    ...
    expect(parseProps(metricPropsSchema, { label: 'x' }, fallback)).toBe(
      fallback,
    );
  });
  ...
  it('returns the fallback when a prop has the wrong type', () => {
    ...
    expect(
      parseProps(tablePropsSchema, { columns: 'nope', rows: [] }, fallback),
    ).toBe(fallback);
  });
  ...
  it('rejects an invalid enum value and falls back', () => {
    ...
    expect(
      parseProps(chartPropsSchema, { type: 'pie3d', data: [] }, fallback),
    ).toBe(fallback);
  });
...
describe('listItemSchema href (model-authored, untrusted)', () => {
  ...
  it('keeps http, https and mailto links', () => {
    expect(parse('https://example.com/invoice/1')).toBe(
      'https://example.com/invoice/1',
    );
    expect(parse('http://example.com')).toBe('http://example.com');
    expect(parse('mailto:support@example.com')).toBe(
  ...
  it('never throws on a bad href — the item still parses, just without a link', () => {
    expect(() =>
      listItemSchema.parse({ label: 'x', href: 'javascript:alert(1)' }),
    ).not.toThrow();
    expect(
      listItemSchema.parse({ label: 'x', href: 'not a url at all' }),
    ).toEqual({
      label: 'x',
      href: null,
    });
