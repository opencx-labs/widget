 ...
 * this same catalog (see `ui-prompt.ts`, codegen-only — never imported by the
 * shipped widget), so the model can never emit a component the widget can't
 * render.
 *
...
export const widgetCatalog = defineCatalog(schema, {
  components: {
    Card: {
      ...
      slots: ['default'],
      description:
        'Container card with an optional title/description. Group related content.',
    ...
    Stack: {
      ...
      slots: ['default'],
      description:
        'Flex layout. Use direction="row" for side-by-side, "column" (default) to stack.',
    },
    Grid: {
    ...
    Text: {
      ...
      description:
        'A paragraph of text. variant "muted"/"caption" for secondary text, "code" for monospace.',
    },
    Badge: {
      props: badgePropsSchema,
      description:
        'Small status pill. Use variant to convey state (success/warning/destructive).',
