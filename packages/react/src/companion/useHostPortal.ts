...
/**
 * A document-root mount point for host-DOM overlays — the companion shell
 * and the page-mark overlays: <Widget> may render arbitrarily deep in
 * the host app (shadow roots, transformed or stacking-context ancestors)
 * where position:fixed breaks. Root-level mounting is the same guarantee
 * the embed script provides.
 *
 * The mount point must sit ABOVE body, not in it: the companion's sidebar
 * layout turns body into the page frame (contain: strict — see
 * app-frame.ts), and containment would trap body-portaled fixed elements
