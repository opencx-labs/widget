...
.ocx-fade-up { animation: ocx-fade-up 0.3s cubic-bezier(0.23, 1, 0.32, 1) both; }
.ocx-placeholder-in { animation: ocx-placeholder-in 0.2s cubic-bezier(0.23, 1, 0.32, 1) both; }
.ocx-placeholder-out { animation: ocx-placeholder-out 0.18s cubic-bezier(0.23, 1, 0.32, 1) both; }
@media (prefers-reduced-motion: reduce) {
  .ocx-text-shimmer, .ocx-fade-up, .ocx-placeholder-in, .ocx-placeholder-out { animation: none; }
}
`;
  ...
  return (
    <span
      className={cn(
        ...
        active &&
          (phase === 'in' ? 'ocx-placeholder-in' : 'ocx-placeholder-out'),
      )}
    >
      {current}
    </span>
...
function ThoughtRow({
  ...
  const displayLabel = isLive
    ? (() => {
        ...
        const last = lines[lines.length - 1] ?? thinkingLabel;
  ...
  return (
    <div className="min-w-0">
      ...
      {isTruncated && open && (
        <div className="ocx-fade-up ms-[30px]">
