...
describe('stripCitationRefs', () => {
  it('strips the model-facing tag form', () => {
    expect(
      stripCitationRefs(
        'Refunds take 5 days.<ref id="knowledge:2"/> Anything else?',
      ),
    ).toBe('Refunds take 5 days. Anything else?');
  });
  ...
  it('strips multiple tags in one text', () => {
    expect(
      stripCitationRefs(
        'A<ref id="knowledge:0"/> and B<ref id="instruction:1"/>.',
      ),
    ).toBe('A and B.');
  });

  it('strips a partial tag cut mid-stream at the end of the buffer', () => {
    expect(stripCitationRefs('The answer is 5 days.<ref id="know')).toBe(
      'The answer is 5 days.',
    );
    expect(stripCitationRefs('The answer is 5 days.<re')).toBe(
      'The answer is 5 days.',
    );
  });
  ...
  it('strips a stray complete <cite/> line outside a block', () => {
    expect(stripCitationRefs('A.<cite id="knowledge:0" used="yes"/> B.')).toBe(
      'A. B.',
    );
  });
});
