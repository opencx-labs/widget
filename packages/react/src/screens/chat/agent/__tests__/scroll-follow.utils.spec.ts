...
import { SCROLL_NEAR_BOTTOM_PX, isNearBottom } from '../scroll-follow.utils';
...
describe('isNearBottom', () => {
  ...
  it('honors a custom threshold on both sides of the boundary', () => {
    ...
    expect(isNearBottom(metrics(550), 60)).toBe(true);
    expect(isNearBottom(metrics(550), 40)).toBe(false);
    // The custom threshold is also strict `<`: distance 50 with threshold 50
    expect(isNearBottom(metrics(550), 50)).toBe(false);
  });
  ...
  it('defaults the threshold to the exported companion-parity constant', () => {
    ...
    expect(isNearBottom(metrics(450))).toBe(
      isNearBottom(metrics(450), SCROLL_NEAR_BOTTOM_PX),
    );
  });
});
