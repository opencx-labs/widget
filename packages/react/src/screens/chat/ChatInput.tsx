...
export function ChatInput({
  ...
  // The tray above the composer carries the page alone. Marks are pictures
  // of the page, so they preview inside the composer with the files.
  const hasAttachedContext = pageEntity !== null && !pageEntityDismissed;
  ...
  return (
    ...
    >
      ...
      {/* Attached context fused to the composer (Linear-style): one tray
          ...
          it: the composer wears the shared `--opencx-inset-radius` (the
          shell's radius minus this root's padding, declared once in
          `FrameDocument`) and the tray's is the composer's plus the tray
          padding. `ps-3` puts the context title on the composer's TEXT inset
          (4px tray + 12px = the composer's 8px padding + the textarea's 8px),
          not on its edge.
      ...
      <div
        ...
        style={{ '--cx-p': '0.25rem' } as React.CSSProperties}
        className={cn(
          'flex flex-col',
          ...
          'rounded-[calc(var(--opencx-inset-radius)+var(--cx-p))] p-[var(--cx-p)]',
      ...
      >
        ...
        <AnimatePresence>
          {hasAttachedContext && (
            <MotionDiv__VerticalReveal key="attached-context">
              <div
                {...dc('chat/input_box/page_context_container')}
        ...
        <div
          ...
          className={cn(
            ...
            'relative rounded-[var(--opencx-inset-radius)] flex flex-col gap-2 p-2',
          )}
        >
          <div
                    ...
                    >
                      <Button
                        ...
                        className={cn(
                          ...
                          // Armed: click again (or Esc) to disarm.
                          marking.isActive && COMPOSER_TOOL_ARMED,
                        )}
                      >
