...
const GAP = 8;
/** The menu and its detail card are one surface, split in two. */
const MENU_SURFACE = 'rounded-xl border border-border bg-background shadow-lg';
  ...
  return createPortal(
    ...
    >
      <div
        ...
        className={cn('flex max-h-72 flex-col overflow-hidden', MENU_SURFACE)}
      >
      ...
      {detail && placement.detailBeside && <MentionDetail item={detail} />}
    </div>,
    host,
  );
}
...
function MentionDetail({ item }: { item: WidgetMention }) {
  return (
    <div
      {...dc('chat/input_box/mention_picker/detail')}
      style={{ width: DETAIL_WIDTH }}
      className={cn('flex gap-3 p-3', MENU_SURFACE)}
    >
