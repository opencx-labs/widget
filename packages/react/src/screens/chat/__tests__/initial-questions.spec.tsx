...
const sendMessage = vi.fn(async ({ content }: { content: string }) => {
  messages = [{ id: 'question', type: 'USER', content, timestamp: null }];
});
...
vi.mock('@opencx/widget-react-headless', () => ({
  ...
  useHostLocation: () => 'https://example.test',
  useComposerDraft: () => useComposerDraft(),
  useAgentChatUi: () => ({ isStreaming: false, queuedUserMessages: [] }),
  useDictation: () => ({ enabled: false, stop: vi.fn() }),
...
vi.mock('../usePageMarkComposer', () => ({
  usePageMarkComposer: () => ({ marks: [], marking: {} }),
}));
...
describe('required initial questions', () => {
  ...
  it('reacts to configuration changes without losing the composer draft', () => {
    config.requireInitialQuestion = false;
    draftState.setPartial({ text: 'My draft' });
    render();
