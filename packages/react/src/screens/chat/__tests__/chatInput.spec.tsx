...
vi.mock('../../../page-marks/PageMarkPill', () => ({
  // Where the pill lands is asserted; what it draws is its own spec's job.
  PageMarkPill: () => <span data-component="chat/input_box/page_mark_pill" />,
}));
...
describe('ChatInput send acceptance', () => {
  ...
  it('docks attached context outside the composer so the box never grows', async () => {
    ...
    const pills = container.querySelector(
      '[data-component="chat/input_box/page_context_container"]',
    );
    if (!composer || !pills)
  ...
  it('previews a mark inside the composer with the files, not in the tray', async () => {
    marks = [{ shape: 'box', elements: [{ name: 'table "Payments"' }] }];
    allFiles = [];
    await renderInput();
    ...
    if (!composer || !pill) throw new Error('composer or mark did not render');
    // A mark is a picture of the page: it sits where the pictures sit.
    expect(composer.contains(pill)).toBe(true);
    expect(
      pill.closest('[data-component="chat/input_box/attachments_container"]'),
    ).not.toBeNull();
    // Nothing but the page belongs in the tray; a mark alone leaves it empty.
    expect(
