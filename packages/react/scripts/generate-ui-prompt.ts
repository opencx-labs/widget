 ...
 *
 * The agent chat model needs the inline-mode UI prompt server-side so it emits
 * JSONL patches the stream's `pipeJsonRender` can split into `data-spec`
...
import { widgetUiPrompt } from '../src/json-render/ui-prompt';
...
// Source: widget repo — packages/react/src/json-render/ui-prompt.ts (\`widgetUiPrompt\`)
