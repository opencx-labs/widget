...
- `mentions.search`: the visitor types `@` and picks from the host's own items; each rides the send as `clientContext.mentions` and shows as a chip on the composer and the sent bubble.
