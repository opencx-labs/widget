...
describe('AgentChatQueue', () => {
  ...
  it('rejects the newest item past the bound and preserves accepted FIFO state', () => {
  ...
  it('dequeueNext() is null on an empty queue', () => {
    expect(new AgentChatQueue<string>(5).dequeueNext()).toBeNull();
  });

  it('treats a non-positive bound as a one-item accepted backlog', () => {
